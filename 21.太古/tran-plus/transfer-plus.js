    ...
    transfer.prototype = {
        ...
        initContainer: function () {
            ...
            var containerHtml = ['<div class="col-sm-5 transferBox">',
                ...
                '<div class="btnList">',
                '<span class="btn forwardBtn" >加入</span>',
                '<span class="btn allForwardBtn" >全加入</span>',
                '<span class="btn allBackwardBtn" >全取消</span>',
                '<span class="btn backwardBtn" >取消</span>',
                '<span class="btn upBtn" >向上</span>',
                '<span class="btn downBtn" >向下</span>',
            ...
            this.$allForwardBtn = this.$el.find('.allForwardBtn'); //向待选表格内添加的按钮
            this.$upBtn = this.$el.find('.upBtn'); //向上
            this.$downBtn = this.$el.find('.downBtn'); //向下
            ...
            this.$allBackwardBtn = this.$el.find('.allBackwardBtn'); //向待选表格内添加的按钮
        ...
        initoption: function () {
            ...
            this.option.selectColumns[0].field = this.option.diffKey;
        ...
        initEvent: function () {
            ...
            // 全加入
            this.$allForwardBtn.click(function () {
                _this.allForwardData();
            });
            // 取消
            ...
            // 全取消
            this.$allBackwardBtn.click(function () {
                _this.allBackwardData();
            });
            // 向上
        ...
        // 全取消
        allBackwardData: function () {
            this.unselectData = this.unselectData.concat(this.selectData)
            this.selectData = []
            this.refreshTable();
        },
        // 全加入
        allForwardData: function () {
            this.selectData = this.selectData.concat(this.unselectData)
            this.unselectData = []
            this.refreshTable();
        },
        /*
        ...
        upData: function () {
            var selectedList = this.getSelect(this.$selectTable.find('tbody tr'));
            selectedList.forEach(item_id => {
                this.selectData.forEach((item, index) => {
                    if (item_id === item.importUnitId && index !== 0) {
                        var temp = this.selectData[index - 1]
                        this.selectData[index - 1] = item
                        this.selectData[index] = temp
                    }
                })
            })
            this.refreshTable();
        ...
        downData: function () {
            ...
            selectedList.forEach(item_id => {
                this.selectData.forEach((item, index) => {
                    if (item_id === item.importUnitId && index < this.selectData.length-1) {
                        indexNum.push(index)
                    }
                })
            })
            for (let index = indexNum.length - 1; index >= 0; index--) {
                var temp = this.selectData[indexNum[index] + 1]
                this.selectData[indexNum[index] + 1] = this.selectData[indexNum[index]]
                this.selectData[indexNum[index]] = temp
            }
        ...
        transferData: function ($dom, type) {
            ...
            if (type) {
                ...
                if (!this.option.singleSelect && (this.option.maxSelect - 0) && typeof (this.option.maxSelect - 0) == "number") {
                ...
                for (var i = 0; i < this.unselectData.length; i++) {
                    if (selectList.indexOf(this.unselectData[i][this.option.uniqueId]) >= 0) {
                        ...
                        if (this.option.singleSelect && this.selectData.length > 0) { // 单选
                            var selectDataCopy = JSON.parse(JSON.stringify(this.selectData[0]));
                            this.selectData.splice(0, this.selectData.length);
                            this.selectData.push(this.unselectData[i]);
                            this.unselectData.splice(i, 1);
                            this.unselectData.push(selectDataCopy);
                        } else { // 多选
                            this.selectData.push(this.unselectData[i]);
                            this.unselectData.splice(i, 1);
                        }
